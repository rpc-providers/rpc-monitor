...
declare -A network_to_chain=(
  ...
  [kusama]="ksmcc3"
    ...
    start_time=$(date -d "2025-04-15" +%s)
    end_time=$(date -d "2025-07-15" +%s)
    report_date="Quarterly Report (Apr 15 – Jul 15)"
...
fetch_rpc_calls() {
    ...
    result=$(curl -s -G "$PROMETHEUS_SERVER/api/v1/query" \
        --data-urlencode "query=sum(increase(substrate_rpc_calls_started{chain=\"$chain\",job=\"$job\"}[90d]))" \
...
echo ""
echo "- **RPC Calls**: RPC calls in the last 90 days in millions"
echo ""
